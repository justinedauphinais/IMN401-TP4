...
bool OBJLoader::loadModel(std::string filename, GeometricModel *model) {
    ...
    if (!fp_in) // file couldn't be opened
        throw std::logic_error(std::string("ERROR : OBJ Geometric Model Loader : ") + filename + std::string(" could not be opened or does not exist.\n"));
    ...
    while (!fp_in.eof()) {
        ...
        } else if (line.find("f") == 0) // Faces
        {
            ...
            do {
                ...
                if (pp != -1) {
                    std::string segmentVt = segment.substr(pp + 1, std::string::npos);
            ...
            if (Vlist_per_face.size() != 3)
                throw std::logic_error("ERROR : Geometric Model : " + filename + " is not a triangle-based mesh.\n");
...
void OBJLoader::setupForTextureCoordinates(GeometricModel *model) {
    ...
    for (int i = 0; i < model->nb_faces; i++) {
        ...
        else if (packcoords[(int)c_face.s1] != ctex) { // deja un vertex assigne : On dedouble le sommet
        ...
        else if (packcoords[(int)c_face.s2] != ctex) // deja un vertex assigne : On dedouble le sommet
        {
            packcoords.push_back(ctex);
        ...
        else if (packcoords[(int)c_face.s3] != ctex) // deja un vertex assigne : On dedouble le sommet
        {
            packcoords.push_back(ctex);
